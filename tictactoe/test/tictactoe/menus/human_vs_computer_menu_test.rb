...
class HumanVsComputerMenuTest < Minitest::Test
  def setup
    ...
    @human_vs_computer_menu = HumanVsComputerMenu.new(@io_device)
  end

  def test_human_vs_computer_menu
    @human_vs_computer_menu.symbol_menu = @symbol_menu
    @human_vs_computer_menu.difficulty_menu = @difficulty_menu

    player_hash = @human_vs_computer_menu.run_menu
  ...
  def test_raise_error_when_difficulty_menu_not_set
    ...
    assert_raises IllegalStateError do
      @human_vs_computer_menu.run_menu
    end
  end
